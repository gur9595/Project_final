...
public class PagingUtil {
   ...
   public static String pagingImg(int totalRecordCount, int pageSize, int blockPage, int nowPage, String page) {
      ...
      if (intTemp != 1) {
         ...
         // 두번째 페이지 블럭부터 출력됨.
         pagingStr += "" + "<a href='javascript:paging(1);'>" + "<i class='fas fa-angle-double-left'></i></a>";
         pagingStr += "&nbsp;";
      ...
      if (intTemp <= totalPage) {
         pagingStr += "<a href='javascript:paging("+intTemp+");'>" + "<i class='fas fa-angle-right'></i></a>";
         pagingStr += "&nbsp;";
         pagingStr += "<a href='javascript:paging("+totalPage+");'>" + "<i class='fas fa-angle-double-right'></i></a>";
      }

      return pagingStr;
   }
   
   public static String paging(int totalRecordCount, int pageSize, int blockPage, int nowPage, String page) {
   ...
   public static String pagingImgFilter(int totalRecordCount, int pageSize, int blockPage, int nowPage, String page) {
      ...
      if (intTemp != 1) {
         // 첫번째 페이지 블럭에서는 출력되지 않음
         // 두번째 페이지 블럭부터 출력됨.
         pagingStr += "" + "<a href='javascript:pagingFilter(1);'>" + "<i class='fas fa-angle-double-left'></i></a>";
         pagingStr += "&nbsp;";
         pagingStr += "" + "<a href='javascript:pagingFilter("+(intTemp-blockPage)+");'>"
               + "<i class='fas fa-angle-left'></i></a>";
      }
      ...
      while (blockCount <= blockPage && intTemp <= totalPage) {
         if (intTemp == nowPage) {
            ...
            pagingStr += "&nbsp;<a href='javascript:pagingFilter("+intTemp+");'>" + intTemp + "</a>&nbsp;";
         }
         intTemp++;
         blockCount++;
      }
      ...
      // 5.다음페이지블럭 & 마지막페이지 바로가기
      if (intTemp <= totalPage) {
         pagingStr += "<a href='javascript:pagingFilter("+intTemp+");'>" + "<i class='fas fa-angle-right'></i></a>";
         pagingStr += "&nbsp;";
         pagingStr += "<a href='javascript:pagingFilter("+totalPage+");'>" + "<i class='fas fa-angle-double-right'></i></a>";
      }
